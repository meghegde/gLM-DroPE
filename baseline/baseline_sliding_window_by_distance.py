...
FINETUNED_MODEL = "ntv2-qk_norm_nope-lr=5e-05-2048-bp-seed=42"
...
class SlidingWindowWrapper(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, labels=None):
        ...
        logits = logits.float()   # safer for metrics
...
ds_1 = dataset.filter(lambda x: x["distance_to_nearest_tss"] in list(range(0, 30000)))
ds_2 = dataset.filter(lambda x: x["distance_to_nearest_tss"] in list(range(30000, 100000)))
ds_3 = dataset.filter(lambda x: x["distance_to_nearest_tss"] >=100000)
